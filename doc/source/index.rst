.. Libre-Printer documentation master file, created by
   sphinx-quickstart on Sun Jun 26 20:42:37 2022.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

Overview of Libre-Printer |release|
===================================

|project_name| is a free reimplementation of the Retroprinter project and its
proprietary interface.
The idea is to provide a software and hardware interfaces pretending to be a
printer that is compatible with old medical and industrial devices (machine tools).

Sometimes these devices cannot be replaced for cost reasons or simply because
there is no justification to do so.
However, they often work in pairs with printers that are no longer manufactured
and are often the weak point of the installation because they are prone to
breakdowns and the abandonment of the manufacture of their consumables.


Basics
------

.. toctree::
   :maxdepth: 2

   project_presentation
   installation
   service_configuration
   examples
   interface_usage

   FAQ
   license


Project Info
------------

.. toctree::
   :maxdepth: 2

   dev_overview


API Reference
-------------

.. toctree::
   :maxdepth: 3

   dev_work_with_source


Indices and tables
==================

* :ref:`genindex`
* :ref:`modindex`
